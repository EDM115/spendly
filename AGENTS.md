...
- Run commands as `pnpm <command>`. If adding/editing dependencies, update `package.json` with the tilde (`~`) version range and run `pnpm up && pnpm dedupe` to update the lockfile (do not use `pnpm i` or `pnpm add`).
